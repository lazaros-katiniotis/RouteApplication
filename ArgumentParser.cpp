...
void ArgumentParser::CreateStateTable() {
	...
	stateTable_->SetState(ParserState::FILE_STATE, InputState::COORDINATE, ParserState::START_STATE);
}

void ArgumentParser::ParseArgument(string_view arg) {
	UpdateCurrentState(arg);

	UpdatePreviousState();
}

void ArgumentParser::UpdateCurrentState(string_view arg) {
	cout << "Parsing argument: " << arg << endl;
	static int number_of_coordinates_left_to_parse = -1;
	if (arg == "-b") {
	...
	cout << "current Parser state: " << (int)currentParserState_ << ", current input state: " << (int)currentInputState_ << endl;
	cout << "number_of_coordinates_left_to_parse: " << number_of_coordinates_left_to_parse << endl;

	if (number_of_coordinates_left_to_parse == 0) {
		currentParserState_ = static_cast<ParserState>(stateTable_->GetState(currentParserState_, currentInputState_));
		cout << "updated current Parser state: " << (int)currentParserState_ << ", updated current input state: " << (int)currentInputState_ << endl;
	}

	if (currentParserState_ == ParserState::PARSING_STATE && currentInputState_ == InputState::COORDINATE) {
		query_ += arg;
		number_of_coordinates_left_to_parse--;
		if (number_of_coordinates_left_to_parse != 0) {
			query_ += ",";
		}
	}
	...
	case ParserState::PARSING_STATE:
		...
		case InputState::BOUNDS_COMMAND:
			number_of_coordinates_left_to_parse = bound_coordinates_ - 1;
			cout << "Will need to parse the next " << number_of_coordinates_left_to_parse << " coordinates." << endl;
			break;
		case InputState::START_POINT_COMMAND:
			number_of_coordinates_left_to_parse = point_coordinates_ - 1;
...
void ArgumentParser::UpdatePreviousState() {
	previousParserState_ = currentParserState_;
	previousInputState_ = currentInputState_;
}
